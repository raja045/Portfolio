window.addEventListener("DOMContentLoaded", async function () {
    ...
    document.querySelectorAll(".stack-card").forEach(async function (el) {
        ...
        const response = await get(`https://tryhackme.com/api/v2/badges/public-profile?userPublicId=${userPublicId}`);
    ...
    // document.querySelectorAll(".stack-card").forEach(async function (el) {
    //     const userPublicId = el.getAttribute("user-id"); // Replace with the attribute holding your user ID
    
    //     try {
    //         // Replace this URL with the correct API endpoint for TryHackMe
    //         const response = await fetch(`https://tryhackme.com/api/v2/badges/public-profile?userPublicId=${userPublicId}`);
    //         const data = await response.json();
    
    //         const { username, rank, points } = data; // Adjust based on API response structure
    
    //         el.innerHTML = `
    //         <div style="font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif,Apple Color Emoji,Segoe UI Emoji; border-radius: 6px; line-height: 1.5; padding: 16px; font-size: 14px; color: #24292e; background-color: #f9bf3f;">
    //             <div style="display: flex; align-items: center;">
    //                 <img style="width: 48px; height: 48px; border-radius: 50%" src="https://tryhackme-badges.s3.amazonaws.com/${username}.png" alt="Profile badge"></img>
    //                 <div style="display: flex; flex-direction: column; margin-left: 12px">
    //                     <span style="font-weight: 500; color: #000; font-size: 18px">
    //                         <a style="text-decoration: none; color: inherit;" target="_blank" href="https://tryhackme.com/p/${username}">
    //                             ${username}
    //                         </a>
    //                     </span>
    //                     <span style="font-weight: 400; color: #586069; font-size: 12px">
    ...
    //         el.innerHTML = `<p>Unable to load profile. Please check your user ID or API.</p>`;
    //     }
    // });
